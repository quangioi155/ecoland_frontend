...
import { MasterCommon, OemTypeLabel } from 'src/app/common/label.common';
...
/**
 * @export class OemTypeListComponent
 *
 * [2-8-17]Oem Type List Screen
 *
 * author: ITSG-HoanNNC
 */
@Component({
  selector: 'app-oem-type-list',
  templateUrl: './oem-type-list.component.html',
  styleUrls: ['./oem-type-list.component.scss'],
  providers: [ConfirmationService, MessageService]
})
export class OemTypeListComponent implements OnInit, AfterViewInit {
  /* screen title */
  ...
  /* oemName search label */
  public oemNameField: string = OemTypeLabel.OEM_NAME_FIELD;
  ...
  fillData(form: FormGroup) {
    this.oemTypeService.searchOemType(form.value).subscribe(
      (res) => {
        if (
  ...
  delete(id: number) {
    ...
    this.oemTypeService.deleteOemType(req).subscribe(
      (res) => {
  ...
  gotoEdit(id: number) {
    ...
    this.tokenStorageService.saveCacheSearch(
      OemTypeListComponent.name,
      this.cacheSearch
    );
    this.router.navigate(['/ecoland/oem-type-edit', id]);
  }
  ...
  getCacheSearch(): any {
    ...
    this.tokenStorageService.clearCacheSearch(OemTypeListComponent.name);
    return cache;
  }
}
